...
# rake update_work_with_tenant_cname:run['pacific.hyku.docker, lib.hyku.docker']


namespace :update_work_with_tenant_cname do
  desc "Add tenant cname to works created before the feature to add tenant cname
       to work was implemented"

  task :run, [:name] => :environment do |task, args|

    cname_list = Array.wrap(args.to_a)
    ::HykuAddons::AddCnameToExistingWorksJob.perform_later(cname_list)
  end
end
