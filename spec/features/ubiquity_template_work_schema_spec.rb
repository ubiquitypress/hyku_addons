...
RSpec.feature "Create a UbiquityTemplateWork", js: true do
  ...
  let(:license_options) { HykuAddons::LicenseService.new.active_elements.first(2) }
  let(:license) { license_options.map { |h| h["label"] } }
  let(:rights_statement_options) { HykuAddons::RightsStatementService.new.active_elements.first }
  let(:rights_statement) { rights_statement_options["label"] }
  let(:publisher) { ["publisher1", "publisher2"] }
  let(:subject_data_options) { HykuAddons::SubjectService.new.active_elements.first(2) }
  let(:subject_data) { subject_data_options.map { |h| h["label"] } }
  let(:language_options) { HykuAddons::LanguageService.new.active_elements.first(2) }
  let(:language) { language_options.map { |h| h["label"] } }
  ...
  let(:institution_options) { HykuAddons::InstitutionService.new.active_elements.first(2) }
  let(:institution) { institution_options.map { |h| h["label"] } }
  ...
  let(:current_he_institution_options) { HykuAddons::CurrentHeInstitutionService.new.active_elements.first }
  let(:current_he_institution) do
    {
      current_he_institution_name: current_he_institution_options["label"]
    }
  end
  ...
  let(:qualification_name_options) { HykuAddons::QualificationNameService.new.active_elements.first }
  let(:qualification_name) { qualification_name_options["label"] }
  let(:qualification_level_options) { HykuAddons::QualificationLevelService.new.active_elements.first }
  let(:qualification_level) { qualification_level_options["label"] }
  ...
  let(:related_identifier) do
    [
      {
        related_identifier: "123456",
  ...
  let(:refereed_options) { HykuAddons::RefereedService.new.active_elements.first }
  let(:refereed) { refereed_options["label"] }
  ...
  let(:irb_number) { "123" }
  let(:irb_status_options) { HykuAddons::IrbStatusService.new.active_elements.first }
  let(:irb_status) { irb_status_options["label"] }
  ...
  let(:audience_options) { HykuAddons::AudienceService.new.active_elements.first(2) }
  let(:audience) { audience_options.map { |h| h["label"] } }
  ...
  let(:georeferenced_options) { HykuAddons::GeoreferencedService.new.active_elements.first }
  let(:georeferenced) { georeferenced_options["label"] }
  ...
  context "when the form is filled out" do
    before do
      ...
      # fill_in_multiple_selects(:subject, subject_data)
      ...
      # fill_in_cloneable_date_field(:related_exhibition_date, related_exhibition_date)
      ...
      # fill_in_legacy_cloneable_field(:related_identifier, related_identifier)
    ...
    describe "submitting the form" do
      ...
      it "redirects to the work show page" do
        ...
        # expect(work.resource_type).to eq([resource_type])
        # expect(work.date_published).to eq(normalize_dates(date_published).first)
        # # expect(work.creator).to eq([creator.to_json])
        # # expect(work.contributor).to eq([contributor.to_json])
        # expect(work.description).to eq(description)
        # expect(work.keyword).to eq(keyword)
        # expect(work.source).to eq(source)
        # expect(work.license).to eq(license)
        # expect(work.rights_statement).to eq(rights_statement_options.map { |opt| opt["id"] })
        # expect(work.publisher).to eq(publisher)
        # expect(work.subject).to eq(subject_data)
        # expect(work.language).to eq(language)
        # expect(work.identifier).to eq(identifier)
        ...
        # expect(work.event_date).to eq(normalize_dates(event_date))
        # expect(work.series_name).to eq(series_name)
        # expect(work.book_title).to eq(book_title)
        # expect(work.editor).to eq([editor.to_json])
        ...
        # expect(work.current_he_institution).to eq(current_he_institution)
        # expect(work.date_accepted).to eq(normalize_dates(date_accepted).first)
        # expect(work.date_submitted).to eq(normalize_dates(date_submitted).first)
        ...
        # expect(work.related_exhibition_date).to eq(normalize_dates(related_exhibition_date))
        # expect(work.rights_holder).to eq(rights_holder)
        # expect(work.related_exhibition_venue).to eq(related_exhibition_venue)
        # expect(work.qualification_name).to eq(qualification_name)
        # expect(work.qualification_level).to eq(qualification_level)
        # expect(work.alternate_identifier).to eq(alternate_identifier)
        # expect(work.related_identifier).to eq(related_identifier)
        # expect(work.refereed).to eq(refereed)
        ...
        # expect(work.irb_status).to eq(irb_status)
        ...
        # expect(work.audience).to eq(audience)
        # expect(work.related_material).to eq(related_material)
        # expect(work.note).to eq(note)
        ...
        # expect(work.georeferenced).to eq(georeferenced)
