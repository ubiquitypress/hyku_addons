...
RSpec.feature "Create a UngTimeBasedMedia", js: true, slow: true do
  let(:work_type) { "ung_time_based_media" }
  ...
  let(:institution_options) { HykuAddons::InstitutionService.new(model: model).active_elements.sample(1) }
  ...
  context "when the form is filled out" do
    before do
      ...
      fill_in_select(:institution, institution_options.map { |h| h["label"] }.first)
