...
RSpec.feature "Create a NsuGenericWork", customer: true do
  ...
  context "when rendering the form" do
    ...
    it "renders all simple worktype fields" do
      worktype_simple_fields = %w[title alt_title resource_type alt_email abstract
                                  keyword subject degree qualification_level qualification_name
                                  advisor committee_member org_unit book_title alt_book_title
                                  buy_book edition volume issue pagination version_number
                                  official_link library_of_congress_classification
                                  dewey mesh adapted_from series_name source isbn
                                  issn eissn publisher place_of_publication event_title event_location
                                  related_exhibition related_exhibition_venue
                                  license rights_holder rights_statement
                                  table_of_contents references medium extent duration
    ...
    it "renders all date fields" do
      worktype_date_fields = %w[date_published date_accepted date_submitted related_exhibition_date
                                event_date]
